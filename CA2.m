...
% N:         dimension of (N x N) square grid
...
A = 6*ones(N,N);    %An initially empty membrane
...
while time < 1    
    ...
    [xdest,ydest] = boundarycheck(xdest,ydest,N);   
    ...
    if type < 4             %(E,S or P)
        ...
        if A(xdest,ydest) == 6      % if destination site is empty...
            A(x,y) = 6;             % ...move molecule to destination site.
            A(xdest,ydest) = type;
            M(mol,type,1) = xdest;
            M(mol,type,2) = ydest;
        ...
        % original site empty
            ...
            switch type
                case {1}                        % if chosen molecule is an E...
                    if A(xdest,ydest) == 2      % ...and destination contains an S...
                        if rand(1) < f          % ...and with probability f...
                            A(xdest,ydest) = 4; % ...place C in destination site...
                            [M,n(4)] = addmol(M,4,xdest,ydest,n(4));
                            A(x,y) = 6;         % ...and clear original site
                            [M,n(1)] = removemol(M,1,mol,n(1));
                            row = findmol(M,2,xdest,ydest,n(2));
                            if row == 0, error = 1
                            else [M,n(2)] = removemol(M,2,row,n(2)); end
                            gamma = gamma + 1;  % update reaction counter
                        end
                    end
                case {2}                        % if chosen molecule is an S...
                    if A(xdest,ydest) == 1      % ...and destination contains an E...
                        if rand(1) < f          % ...and with probability f...
                            A(xdest,ydest) = 4; % ...place C in destination site...
                            [M,n(4)] = addmol(M,4,xdest,ydest,n(4));
                            A(x,y) = 6;         % ...and clear original site
                            [M,n(2)] = removemol(M,2,mol,n(2));
                            row = findmol(M,1,xdest,ydest,n(1));
                            if row == 0, error = 1
                            else [M,n(1)] = removemol(M,1,row,n(1)); end
                            gamma = gamma + 1;  % update reaction counter
                        end
                    end
            end
    ...
    if type == 4
        ...
        near = zeros(1,4);
        if y>1, if A(x,y-1) == 6, near(1)=1; end    % UP
        else if A(x,N) == 6, near(1)=1; end, end
        if y<N, if A(x,y+1) == 6, near(2)=1; end    % DOWN
        else if A(x,1) == 6, near(2)=1; end, end
        if x>1, if A(x-1,y) == 6, near(3)=1; end    % LEFT
        else if A(N,y) == 6, near(3)=1; end, end
        if x<N, if A(x+1,y) == 6, near(4)=1; end    % RIGHT
        else if A(1,y) == 6, near(4)=1; end, end
        ...
        if norm(near,1) > 0     %if there is an empty neighbour site
            ...
            [xdest,ydest] = boundarycheck(xdest,ydest,N);  %check not going off edge
            
            % Remove C from (x,y)
            [M,n(4)] = removemol(M,4,mol,n(4));
            
            q = rand(1);
            
            if q < r + g                         % if q < r+g...
                A(x,y) = 1;                      % ...Place E at (x,y)
                [M,n(1)] = addmol(M,1,x,y,n(1));
            else 
                A(x,y) = 6;                      % ...else (x,y) is made empty.
            end
            ...
            A(xdest,ydest) = newtype;            % Place S, P or C at (xdest,ydest)
            [M,n(newtype)] = addmol(M,newtype,xdest,ydest,n(newtype));
        end
    end
...
%check;     % a routine that verifies the data in matrices A and M agree by the end of 
            % the simulation
